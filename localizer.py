...
class wellStitcher():
	...
	def write_well_img(self):
		...
		cv2.imshow('Stitch', cv2.resize(self.well_img,(int(1024*.8),int(822*.8)),interpolation = cv2.INTER_AREA))
...
class Localizer(QtCore.QObject):
	...
	def __init__(self, parent = None):
		...
		self.localizer_model = load_model(os.path.join(experiment_folder_location,'model2018-12-06_09_18'), custom_objects={'mean_iou': mean_iou})
	...
	def get_voted_output(self, model_img):
		...
		for label_num in range(1, num_features):
			...
			first_label = np.where(labeled_array == label_num, np.ones(labeled_array.shape), np.zeros(labeled_array.shape))
			...
			if green > red:
				fixed_mask[:,:,1] += np.where(labeled_array == label_num, np.ones(first_label.shape), np.zeros(first_label.shape))
			elif red > green:    
				fixed_mask[:,:,0] += np.where(labeled_array == label_num, np.ones(first_label.shape), np.zeros(first_label.shape))
		return fixed_mask
